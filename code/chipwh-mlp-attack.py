...
def plot_ge_convergance(avgge):
    ...
    plt.tight_layout()
    plt.savefig(f"plots-final/chipwh-ge-conv-mlp-{MLP_DIM}.pdf")
...
if __name__=="__main__":
 ...
 # load data
    ...
    mlp = MLP_model.load_from_checkpoint("./experiments-final/experiments-chipwh/mlp-128/mlp-epoch=07-val_ge_mean=0.00.ckpt", input_dim=input_size, mlp_dim=MLP_DIM, num_classes=CLASSES)
    ...
    key_scores = score_keys(mlp_predictions, plaintexts_test[:, TARGET_BYTE], LEAKAGE_MODEL)
    ...
    ge_median, ge_mean = guessing_entropy(mlp_predictions, plaintexts_test[:, TARGET_BYTE], correct_key, num_attack_traces, LEAKAGE_MODEL)
    print(f"The median of GE: {ge_median}.")
    print(f"The mean of GE: {ge_mean}.")

    print("\nCalculating Guessing Entropy Convergance...")
    medge, avgge = guessing_entropy_convergence(mlp_predictions, plaintexts_test[:, TARGET_BYTE], correct_key, LEAKAGE_MODEL, num_attack_traces)
    print(f"GE Convergance of {np.min(avgge)} with {np.argmin(avgge)} traces.")
    plot_ge_convergance(avgge)
    
    save_ge_convergence(avgge,medge, f"{PATH_MLP}avvge-chipwh-mlp.csv")
